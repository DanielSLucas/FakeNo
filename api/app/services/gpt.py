...
class GPTService(BaseService):
  ...
  def __init__(self, service_name, instruction, example, gpt_input) -> None:
    super().__init__(f"GPT_{service_name}")
    self.intruction = instruction
    self.example = example
    self.gpt_input = gpt_input
  
  def execute(self):
    self.get_completion()
    self.result = self.gpt_output
    return self.gpt_output
  ...
  def get_completion(self):  
    response = openai.ChatCompletion.create(
      ...
      messages=[
        ...
        {"role": "user", "content":self.gpt_input},
      ]
    )

    self.gpt_output = response.choices[0].message.content 


class GetGoogleQueryService(GPTService):
  instruction = """
  ...
  def __init__(self, text) -> None:
    super().__init__(
      service_name = "GET_GOOGLE_QUERY", 
      instruction = self.instruction, 
      example = self.example, 
      gpt_input = text
    )

class GetAnalysisService(GPTService):
  instruction = """
  Você:
